...
LATEST_VERSION=$(curl --silent "https://raw.githubusercontent.com/SealedSaucer/nitro-sniper/main/package.json" | grep '"version":' | sed -E 's/.*"([^"]+)".*/\1/')
...
function installAll() {
  ...
  git clone https://github.com/SealedSaucer/nitro-sniper nitro-sniper &> /dev/null
  cd nitro-sniper
